...
function build_model_2(args; imgsize = (28,28,1), nclasses = 10)
    cnn_output_size = Int.(floor.([imgsize[1]/8,imgsize[2]/8,32]))

    return Chain(
    # First convolution, operating upon a 28x28 image
    Conv((3, 3), imgsize[3]=>16, pad=(0,0), relu),
    MaxPool((2,2)),
...
function build_model_3(args; imgsize = (28,28,1), nclasses = 10)
    cnn_output_size = Int.(floor.([imgsize[1]/8,imgsize[2]/8,32]))

    return Chain(
    flatten,
    Dense(28^2, 30, sigmoid),
    Dense(30, 10, sigmoid)
    )
end
...
Random.seed!(0)
@time train()
test() # 99.22 model_1, 98.53 model_2
